...
6. @annotation, @args : 메서드가 주어진 어노테이션을 가지고 있는 조인 포인트를 매칭
7. bean : 스프링 전용 포인트컷 지시자로, 빈의 이름으로 지정
8. this : 스프링 빈 객체(AOP 프록시)를 대상으로 하는 조인 포인트
9. target : Target 객체(프록시가 가르키는 실제 대상)를 대상으로 하는 조인 포인트
...
### this와 target
this는 프록시 객체를 보고 판단한다.
JDK 동적 프록시로 만들어진 프록시 객체는 인터페이스를 기반으로 만들어진 새로운 클래스다.
따라서 인터페이스의 구현체를 전혀 알지 못하므로, AOP 적용 대상이 아니다.

반면 target은 타겟 객체를 보고 판단한다.
프록시 객체는 구현체를 상속받아 만들어지는 클래스이므로, AOP 적용 대상이 된다.
...
> 그럼 이렇게 내부 호출을 할 때마다 구조를 변경하거나 프록시 빈을 직접 주입해야 할까?

그렇지 않다. 왜냐하면 대부분의 경우 내부 호출 메서드는 `private` 접근제어를 갖는다.
AOP는 프록시 기반이고, 프록시는 메서드 오버라이딩을 기반으로 동작하기 때문에 private 으로 선언된 메서드에는 AOP가 적용되지 않는다.

따라서 private 메서드를 외부 클래스로 변경하고 public 으로 변경하는 경우는 거의 없다.

그러나 가끔 public 메서드에서 public을 호출하는 상황에서 이러한 AOP 미적용 문제가 발생하므로, 꼭 인지하고 있는 것이 좋다.
...
- 따라서 구체 클래스로 타입 캐스팅 시도 시, ClassCastingException이 발생한다.

- CGLIB 동적 프록시는 구체 클래스 기반이다.
- 따라서 구체 클래스나 인터페이스로 타입 캐스팅을 해도 아무런 문제가 없다.
...
이를 그림으로 표현하면 다음과 같다.
![img.png](src/main/resources/image/img_14.png)
...
![img_1.png](src/main/resources/image/img_15.png)

> 여기까지 보면 CGLIB를 사용하는 것이 좋아 보인다. CGLIB를 사용하면 인터페이스 기반이든,
> 클래스 기반이든, 이러한 고민을 할 필요가 없다.
> <br>BUT!, 이런 CGLIB도 단점이 있다.

---

### CGLIB의 한계
**CGLIB는 구체 클래스를 상속 받는 객체를 생성하기 때문에 다음과 같은 문제가 있다.**

1. 대상 클래스에 `기본 생성자` 필수
2. `생성자 2번 호출` 문제
    - 실제 target의 객체를 생성할 때 
    - 프록시 객체를 생성할 때 부모 클래스의 생성자 호출(상속받은 자식 클래스이므로)
3. `final` 키워드 클래스, 메서드에 사용 불가

![img_2.png](src/main/resources/image/img_16.png)

> 그럼 이러한 문제를 스프링은 어떻게 해결할까?
...
`objenesis`라는 특별한 라이브러리를 사용해 기본 생성자 없이 객체 생성이 가능해졌다.
참고로 이 라이브러리는 생성자 호출 없이 객체를 생성할 수 있게 해준다.
2. 생성자 2번 호출 문제
이것도 스프링 4.0 부터 해결되었다.
`objenesis` 라이브러리를 이용해 생성자가 1번만 호출되도록 개선하였다.
   (타겟 클래스 생성 시점 1번)
3. final class, method
이건 해결되진 않았는데 AOP를 적용하는 클래스나 메서드에 final을 사용하는 경우는 매우 적으므로 신경쓰지 않아도 된다.